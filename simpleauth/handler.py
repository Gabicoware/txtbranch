...
class SimpleAuthHandler(object):
  ...
  PROVIDERS = {
    ...
    'reddit'     : ('oauth2', 
      'https://ssl.reddit.com/api/v1/authorize?{0}',
...
#this is obviously too specific

    use_http_basic = provider == 'reddit'

    if use_http_basic:
        headers['Authorization'] = b'Basic ' + base64.b64encode(client_id + b':' + client_secret)
    else:
  ...
  def _get_reddit_user_info(self, auth_info, key=None, secret=None):
    """Returns a dict of currenly logging in user.
    Reddit API endpoint:
    ...
    #link is a standard field
    data['link'] = 'http://www.reddit.com/u/'+data['name']
